...
import collections
from load_data import SQLiteExtractor, Movie, Genre, Person, \
    PersonFilmWork, GenreFilmWork


def get_test(data_sl, data_pg):
    for x in data_pg:
        pg_list = x.get_list_no_date()
        sl_list = data_sl[data_pg.index(x)].get_list_no_date()
        if collections.Counter(pg_list) != collections.Counter(sl_list):
            return False
    return True
...
class TestSumFunc:
    ...
    def test_len_genre_film_work(self, sqlite_curs, curs_pg):
        ...
        sql_data = SQLiteExtractor(None, curs_pg)
        data_pg = sql_data.extract_data('*', 'content.genre_film_work',
                                        GenreFilmWork)
    ...
    def test_len_person_film_work(self, sqlite_curs, curs_pg):
        sql_data = SQLiteExtractor(None, sqlite_curs)
        data_sl = sql_data.extract_data('*', 'person_film_work',
                                        PersonFilmWork)

        sql_data = SQLiteExtractor(None, curs_pg)
        data_pg = sql_data.extract_data('*', 'content.person_film_work',
                                        PersonFilmWork)
